...
IAM_ROLE_ARN=$(aws iam create-role --role-name $ROLE_NAME --assume-role-policy-document '{"Version": "2012-10-17","Statement": [{"Sid": "","Effect": "Allow","Principal": {"Service": "lambda.amazonaws.com"},"Action": "sts:AssumeRole"}]}'|jq -r ".Role.Arn")
...
## Systems Manager - EC2 Greengrass Display
### upload certs
- cloud9 to upload core certs
```
scp xxx-setup.tar.gz root@10.20.1.220:~
```
### install certs
...
- run py
```
python3 ratchet_display.py -t /ratchet-lcd -e 10.20.1.200 -c xxx.cert.pem -k xxx.private.key -r RatchetG01_Core_CA.pem
```

## Systems Manager - EC2 Greengrass Button
### upload certs
- cloud9 to upload core certs
```
scp xxx-setup.tar.gz root@10.20.1.210:~
```
### install certs
...
- run py
```
python3 ratchet_button.py -t /ratchet -e 10.20.1.200 -c xxx.cert.pem -k xxx.private.key -r RatchetG01_Core_CA.pem
```
